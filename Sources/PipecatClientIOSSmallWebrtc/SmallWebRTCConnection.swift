...
final class SmallWebRTCConnection: NSObject {
    ...
    private var dataChannelOpenHandler: (() -> Void)?
    ...
    required init(iceServers: [String], enableCam: Bool, enableMic: Bool) {
        ...
        guard
            let peerConnection = SmallWebRTCConnection.factory.peerConnection(
                with: config,
    ...
    func getCurrentCamera() -> Device? {
        ...
        guard
            let currentDevice = capturer.captureSession.inputs.compactMap({ ($0 as? AVCaptureDeviceInput)?.device })
                .first
        else {
            return nil
        }
...
extension SmallWebRTCConnection: RTCPeerConnectionDelegate {
    ...
    func peerConnection(_ peerConnection: RTCPeerConnection, didRemove stream: RTCMediaStream) {
        ...
        if !stream.audioTracks.isEmpty && self.remoteAudioTrack != nil
            && self.remoteAudioTrack?.trackId == stream.audioTracks[0].trackId {
            self.remoteAudioTrack = nil
            self.delegate?.onTracksUpdated()
        }
    }
...
extension SmallWebRTCConnection: RTCDataChannelDelegate {
    ...
    func dataChannelDidChangeState(_ dataChannel: RTCDataChannel) {
        ...
        if dataChannel.readyState == .open {
            // Call the handler when the channel opens
            dataChannelOpenHandler?()
            // Clear the handler after it's called
            dataChannelOpenHandler = nil
        }
    }
    ...
    public func waitForDataChannelOpen() async throws {
        return try await withCheckedThrowingContinuation { continuation in
            if self.signallingDataChannel?.readyState == .open {
                continuation.resume()
                return
            }

            // Set up an observer or callback to be notified when data channel opens
            dataChannelOpenHandler = {
                continuation.resume()
            }
        }
...
extension SmallWebRTCConnection {
    ...
    func filterCodec(kind: String, codec: String, in sdp: String) -> String {
        ...
        guard let codecRegex = try? NSRegularExpression(pattern: codecPattern),
            ...
            let mediaRegex = try? NSRegularExpression(pattern: mediaPattern)
        else {
            return sdp
        }
